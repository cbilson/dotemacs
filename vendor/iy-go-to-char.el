...
;;           string
;;
;;    C-w -- quit and kill region between start and current point. If
;; region is
;;           activated before search, then use the original mark
;; instead of the
;;           start position.
;;
;;    M-w -- quit and save region between start and current point. If
;; region is
;;           activated before search, use the mark instead of start
;; position.
;;
...
"
  ...
  (let ((count (if (zerop n) 1 n))
    ...
    (save-excursion
      ...
      (setq pt (match-end 0))
            (setq iy-go-to-char-last-char char) ;; save char only if
     ;; success
            (while cont
              (setq ev (read-event))
